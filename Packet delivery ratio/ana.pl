...
if ($logfile eq "" || $logfile eq "-" ){
	open ($fh_pipedlog,'>', "COOJA.testlog") or die $!; 
	foreach $line (<STDIN>) {
		print $fh_pipedlog $line; 
	}
	close $fh_pipedlog;
	# update filelog
	$logfile="COOJA.testlog";
	print "Received log from pipe.\n";
} 


# call to the required function   
 networksetuptime($logfile);
 #energy($logfile);
 networktraffic($logfile);
 networklatency($logfile);
...
sub networklatency {
	...
	foreach $line(<$fh_logfile>){	
		if ($line =~ m/(\d+):(\d+):DATA send to 1 'Hello (\d+)'/)  { # line can be either sending or receiving
			...
			if ($line =~ m/(\d+):\d+:DATA recv 'Hello (\d+) from the client' from 2/) {                        
				 $recv = $recv + 1;
				$nodenr=$2; $packetnr=$3; $time = $1; # save nodenr,packetnr,time		
...
#					# printf "latency for node:$nodenr, packet:$packetnr = %d\n", $time - $sendTime;
	...
	#print "Hello, world, from " . $counter . ".\n";
       # print "loss packets";
#	printf "lost packets are:\n";
	$lostpackets = printLostPackets();
	printf "NETWORK LATENCY\n";
	printf "=================\n";
	$row = sprintf  "Packet delivery ratio\tno of SendPackets\tLost Packets\n %-2f %-23d %-15d\n" ,($recv/$noSendPackets)*100, $noSendPackets, $recv;
	printf $row; 
